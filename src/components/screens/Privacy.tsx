  ...
  return (
    <AppLayout header={header}>
      <div className="p-4 text-center">
        <Circ
          size={72}
          className="mx-auto my-2 border-none bg-[var(--av-red-tint)] text-[24px] text-[var(--av-red)]"
        >
          ✓
        </Circ>
      ...
      <div className="px-4">
        <Box className="my-1 p-3">
          ...
          <button
            className="mt-2 cursor-pointer rounded-none border border-[var(--av-text)] px-3 py-1 text-[9px] font-bold uppercase hover:bg-[var(--av-red-tint)]"
            onClick={() => {
      ...
      <div className="px-4">
        ...
        {s.reveals.slice(0, 3).map((r) => (
          <div
            key={r.id}
            className="flex justify-between border-b-2 border-[var(--av-divider)] py-2 text-[11px]"
          >
        ...
        <button
          className="mt-3 w-full cursor-pointer rounded-none border border-[var(--av-text)] py-2 text-[10px] font-bold hover:bg-[var(--av-red-tint)]"
          onClick={() => s.navigate({ name: "backup-verify" })}
        >
          Recovery Phrase →
        </button>
      </div>
